...
# -----------------------------
# Prompt generator (non-COT)
# -----------------------------
'''def generate_prompt(selected_segment, segment_length=100, prediction_length=5):
    ...
    prompt = (
        ...
        f"      {segment_str}. "
        f"This is a hypothetical projection; considering both the plot and time-series values, "
        f"make a speculative prediction about the stock price value for the next "
        f"{prediction_length} days approximately. only give the {prediction_length} next prices."
    )
    return prompt'''

def generate_prompt(selected_segment, segment_length=100, prediction_length=5):
    """
    Generates a multi-line prompt with:
      - Intro referencing the first 'segment_length' months
      - The full 'selected_segment' array
      - A request to predict the next 'prediction_length' months.
    """
    # Convert the array to a nicely formatted string with indentation
    segment_str = np.array2string(
        np.array(selected_segment),
        precision=7,             # 7 decimal places (tweak as needed)
        separator=', ',
        max_line_width=80,       # Wrap lines near 80 chars
        suppress_small=False,
        prefix='      '          # Indentation for continued lines
    )

    # Construct the final prompt
    prompt = f"""this is the plot of a stock over the first {segment_length} days, and these are the time-series values:
      {segment_str}. Considering both the plot and time-series values, predict the stock price for the next {prediction_length} days approximately. only give the {prediction_length} next prices."""
...
# -----------------------------
# MAIN LOOP USING generate_prompt
# -----------------------------
if __name__ == "__main__":
    ...
    for i in range(100, len(stock_list) - (segment_length + prediction_length), step_forward):
        print("Index:", i)
        ...
        prompt = generate_prompt(selected, segment_length, prediction_length)
